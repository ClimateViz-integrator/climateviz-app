...
class chatTest(unittest.TestCase):
    ...
    def test_CB_01(self):
        ...
        response = chat_endpoint("¿Cómo será el clima para Manizales dentro de 2 días?", db=self.db)
    ...
    def test_CB_02(self):
        ...
        self.assertEqual(response.json()['response'], "¿Para qué ciudad quieres saber el clima? No pude identificar la ciudad en tu mensaje.")
    ...
    def test_CB_03(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()['response'], "¿Cuántos días quieres saber el clima? No pude identificar los días en tu mensaje.")
    
    def test_CB_04(self):
        """ CB-04: en el message esta vacio → 400"""
        response = chat_endpoint("", db=self.db)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()['response'], "No entendí bien la pregunta. Por ejemplo, pregunta: '¿Cómo estará el clima en Madrid mañana?' o '¿Lloverá en Barcelona en los próximos 3 días?'")
