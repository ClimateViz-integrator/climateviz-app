...
from Controllers.weather_api import extract_additional_info, extract_hourly_data, get_data
...
class PredictionController:
    ...
    def __init__(self):
        ...
        self.model, self.preprocessor = TrainOrLoadModel().train_or_load_model() # Entrenar o cargar el modelo al iniciar la clase
    ...
    async def predict_from_api(self, city, days, db):
        ...
        df_clima_proc = self.preprocessor.add_cyclical_features(df_clima.copy(), tiempo_s)
        ...
        x_seq, _ = dataset_handler.create_sequences(df_clima_proc, target_col=CONFIG['TARGET_COL'])
        ...
        pred_original = self.preprocessor.inverse_scale_target(pred_scaled, target_names=['temp_c', 'humidity'])
        ...
        future_dates = pd.date_range(start=last_datetime + pd.Timedelta(hours=1), periods=len(pred_original), freq='H')
