...
def get_data(city: str, days: int):
    ...
    if days == 0:
        ...
        for i in range(1, days + 1):
            date = (pd.to_datetime(date_current) - pd.DateOffset(days=i)).strftime(
                "%Y-%m-%d"
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Error en la solicitud para la fecha {date}: {response.status_code}"
                )
...
def extract_additional_info(city: str, days: int):
    ...
    for day_data in forecast_data.get("forecast", {}).get("forecastday", []):
        ...
        info = {
            ...
            "current": (
                forecast_data.get("current", {})
                if day_data.get("date") == datetime.now().strftime("%Y-%m-%d")
                else None
            ),
            ...
            "alerts": (
                forecast_data.get("alerts", {}).get("alert", [])
                if day_data.get("date") == datetime.now().strftime("%Y-%m-%d")
                else []
            ),
